...
def handle_diversified_word(word, on_word):
    ...
    if var_cnt >= 1:
        m = pat.search(word)
        start, middle, end = word[:m.start()], word[m.start("aldono"):m.end("aldono")], word[m.end():]
...
def dictionary_generator(dst_fname, convert_accents=False, css_text=None,
    ...
    def make_on_parse(on_parsed_article):
        def on_parse(all_names, lines):
            ...
            # :TODO: вообще-то это лучше вынести в parse_dictionary(), так как
            ...
            if convert_accents:
                ...
                for i in strike_indexes:
                    try:
                        article = article[:i] + article[i+1] + "\u0301" + article[i+2:]
                    except:
